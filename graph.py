...
boardsize = 5
dimensions = 3*boardsize*boardsize+2*boardsize+3

with tf.Session() as sess:
    inp = tf.placeholder("float", [None,dimensions], name="inp")
    rew = tf.placeholder("float", [None,1], name="rew")
    w1 = tf.Variable(tf.random_normal([dimensions,50]), name="w1")
    ...
    tf.train.write_graph(sess.graph_def, 'models/', 'graph_'+str(boardsize)+'.pb', as_text=False)
